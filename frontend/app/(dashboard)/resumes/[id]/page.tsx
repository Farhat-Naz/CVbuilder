...
import { ChevronLeft, ChevronRight, Save, Eye, Download, ArrowLeft, Loader2 } from "lucide-react";
...
export default function ResumeEditPage() {
  ...
  useEffect(() => {
    resumeService.get(id)
      .then(setResumeData)
      .catch(() => { toast.error("Resume not found"); router.push("/resumes"); })
      .finally(() => setLoading(false));
  }, [id]);

  const updateResume = useCallback((updates: Partial<Resume>) => {
    setResumeData((prev) => prev ? { ...prev, ...updates } : updates);
  }, []);
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold font-heading">{resumeData.title || "Edit Resume"}</h1>
